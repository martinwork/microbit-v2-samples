...
void deepsleep_test1(); // A fiber that sleeps for a few seconds
void deepsleep_test2(); // A fiber that sleeps until woken by button A or B
void deepsleep_test3(); // A timer event handler that sleeps between events
void deepsleep_test4(); // Wake to run button A or B handlers
void deepsleep_test5(); // Two timer event handlers with different periods
...
void deepsleep_test_sendTime( const char *suffix);
void deepsleep_test_beep( int freq);
void deepsleep_test_tune();
...
void deepsleep_test1_fiber()
{
    while (true)
    {
        deepsleep_test_sendTime( "deepsleep_test1_fiber\n");
...
{
    deepsleep_test_sendTime( "deepsleep_test2\n");
...
void deepsleep_test2_fiber()
{
    while (true)
    {
        deepsleep_test_sendTime( "deepsleep_test2_fiber\n");
...
{
    deepsleep_test_sendTime( "deepsleep_test3_onTimer\n");
    deepsleep_test_zeroone();
...
{
    deepsleep_test_sendTime( "deepsleep_test3\n");
...
{
    deepsleep_test_sendTime( "deepsleep_test4\n");
    ...
    create_fiber( deepsleep_test4_fiber);
...
// Two timer event handlers with different periods

void deepsleep_test5_onTimer1(MicroBitEvent e)
{
    uBit.power.powerDownDisable();

    deepsleep_test_sendTime( "deepsleep_test5_onTimer1\n");
...
void deepsleep_test5_onTimer2(MicroBitEvent e)
{
    ...
    deepsleep_test_sendTime( "deepsleep_test5_onTimer2\n");
...
{
    deepsleep_test_sendTime( "deepsleep_test5\n");
...
void deepsleep_test_zeroone()
{
    deepsleep_test_sendTime( "deepsleep_test_zeroone\n");
...
void deepsleep_test_threefour()
{
    deepsleep_test_sendTime( "deepsleep_deepsleep_test_zeroonethreefour\n");
...
void deepsleep_test_beep( int freq)
{
    deepsleep_test_sendTime( "deepsleep_deepsleep_test_zeroonebeep\n");

    uint32_t us = floor( 0.5 + 1000000.0 / freq);
    uBit.audio.virtualOutputPin.setAnalogPeriodUs( us);
    uBit.audio.virtualOutputPin.setAnalogValue(64);
    uBit.sleep(60);
...
void deepsleep_test_tune()
{
    deepsleep_test_sendTime( "deepsleep_deepsleep_test_zeroonetune\n");
